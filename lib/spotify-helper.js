...
export async function getTrackAudioFeatures(id) {
    await setSpotifyAccessToken();

    try {
        const audioFeatureResponse = await spotifyApi.getAudioFeaturesForTrack(id);
        return audioFeatureResponse.body;
    } catch (err) {
        console.error("Error retrieving the track audio features: " + err.message);
    }
}